...
#     try:
#         print("Triggering retrieve-secrets.yaml workflow...")
#         subprocess.run(["gh", "workflow", "run", "retrieve-secrets.yaml"], check=True)
#         print("Waiting for secrets to be exported...")
        
#         # Wait for environment variables to be available
#         # This assumes the secrets are exported to the environment
#         api_gateway_url = os.getenv("API_GATEWAY_URL")
#         x_precommit_auth = os.getenv("X_PRECOMMIT_AUTH")
        
#         if not api_gateway_url or not x_precommit_auth:
#             raise ValueError("Secrets not retrieved properly. Ensure the workflow completes successfully.")
        
#         return api_gateway_url, x_precommit_auth
#     except Exception as e:
#         print(f"Error triggering workflow: {str(e)}")
#         sys.exit(1)

# # Retrieve secrets using the workflow
# api_gateway_url, x_precommit_auth = retrieve_secrets()
...
#     headers = {
#         'Content-Type': 'application/json',
#         'X-PreCommit-Auth': x_precommit_auth
#     }
